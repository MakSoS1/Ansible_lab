"""Full-development v19 production refit with fixed post-human weak refresh."""
...
def run_v19_refresh_production(
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("v19 production requires CUDA")
    if expected_split_sha != IMMUTABLE_SPLIT_SHA:
        raise ValueError("v19 production may only use the immutable split SHA")
    if abs(float(refresh_epochs) - 0.05) > 1e-12:
        raise ValueError("v19 refresh_epochs is fixed at 0.05")
    if abs(float(refresh_learning_rate) - 2.0e-6) > 1e-15:
        raise ValueError("v19 refresh_learning_rate is fixed at 2e-6")
    ...
    human_training = train_pair_phase_v18(
        ...
        texts=human_texts,
        device="cuda",
        phase="v19-production-human-all-folds",
        epochs=human_epochs,
        physical_batch_size=32,
        effective_batch_size=32,
        max_length=max_length,
    ...
    refresh_training = train_pair_phase_v18(
        ...
        texts=weak_texts,
        device="cuda",
        phase="v19-production-post-human-weak-refresh",
    ...
    payload: dict[str, object] = {
        "version": "v19-antiforget-production-refit",
        "candidate": "v19-post-human-weak-refresh-rubert",
    ...
    print("V19_REFRESH_PRODUCTION=" + json.dumps(payload, ensure_ascii=False, sort_keys=True), flush=True)
    return payload
