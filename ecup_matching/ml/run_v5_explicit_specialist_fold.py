...
from .v5_explicit_attributes import build_explicit_attribute_features,build_explicit_leaf_cache,learn_explicit_attribute_keys
...
def run(*,items_path:Path,matches_path:Path,manifest_path:Path,output_dir:Path,expected_split_sha:str,held_fold:int,max_keys:int=40,min_support:int=30)->dict:
 ...
 ids=pd.unique(pd.concat([dev['id1'],dev['id2']],ignore_index=True));items=select_items_by_ids(items_path,ids,include_attributes=True);cache=normalize_items(items);leaf_cache=build_explicit_leaf_cache(cache);cats=items.set_index('id')['category'].astype(str);dev['category']=dev['id1'].map(cats)
 train=dev.loc[tm].reset_index(drop=True);valid=dev.loc[vm].reset_index(drop=True);held_rows=rows[vm]
 spec=learn_explicit_attribute_keys(items,train,max_keys_per_category=max_keys,min_support=min_support,item_cache=cache,leaf_cache=leaf_cache)
 ...
 for cat in sorted(valid['category'].astype(str).unique()):
  ...
  xetr=build_explicit_attribute_features(items,trp,spec,item_cache=cache,category=cat,leaf_cache=leaf_cache);xeva=build_explicit_attribute_features(items,vap,spec,item_cache=cache,category=cat,leaf_cache=leaf_cache)
