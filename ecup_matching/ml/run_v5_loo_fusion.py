...
from .v5_fixed_blend import leave_one_out_rank_candidates
from .v5_validation import manifest_sha256


ANCHOR_NAME = "current4_plus_teacher"
ANCHOR_COLUMN = f"candidate_{ANCHOR_NAME}"
FIVE_SIGNAL_COLUMNS = {
    "weak": "source_weak",
...
def run_loo_fusion(
    *,
    ...
    required_columns = (ANCHOR_COLUMN, *FIVE_SIGNAL_COLUMNS.values())
    fixed = align_oof_frame(
        [fixed_oof_path],
        expected_rows=dev_rows,
        expected_folds=folds,
        required_columns=required_columns,
        source_name="fixed_blend_anchor",
    )
    ...
    anchor_scores = fixed[ANCHOR_COLUMN].to_numpy(dtype=np.float64)
    ...
    five_signals = {
        name: fixed[column].to_numpy(dtype=np.float64)
        for name, column in FIVE_SIGNAL_COLUMNS.items()
    }
    candidates = leave_one_out_rank_candidates(five_signals)
    ...
    for name, scores in candidates.items():
        ...
        for fold in sorted(np.unique(folds).tolist()):
            ...
            fold_anchor = float(
                macro_ap_report(fold_frame, anchor_scores[mask])["macro_average_precision"]
            )
            fold_candidate = float(
                macro_ap_report(fold_frame, scores[mask])["macro_average_precision"]
            )
            fold_reports.append(
                {
                    "fold": int(fold),
        ...
        keep_eligible = bool(candidate_ap > anchor_ap and min_fold_delta >= -0.001)
        if keep_eligible:
    ...
    reproduced_prior = float(reports["loo_drop_teacher"]["macro_average_precision"])
    expected_prior = 0.5870570848443828
    if abs(reproduced_prior - expected_prior) > 1e-12:
        raise RuntimeError(
            f"loo_drop_teacher failed to reproduce prior four-signal anchor: "
            f"observed={reproduced_prior}, expected={expected_prior}"
        )

    best_name = max(reports, key=lambda name: reports[name]["macro_average_precision"])
    best_ap = float(reports[best_name]["macro_average_precision"])
    payload = {
        "version": "v5-five-signal-loo",
        "split_sha256": expected_split_sha,
        "development_rows": int(len(dev)),
        "gold_metric_opened": False,
        "gold_rows_scored": 0,
        "target_fitted_blender": False,
        "predeclared_candidate_count": 5,
    ...
    (output_dir / "v5-loo-fusion-metrics.json").write_text(
    ...
    output.to_parquet(output_dir / "v5-loo-fusion-oof.parquet", index=False)
    return payload
...
def main() -> int:
    ...
    parser.add_argument("--fixed-oof", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    parser.add_argument("--expected-split-sha", required=True)
    args = parser.parse_args()
    payload = run_loo_fusion(
        items_path=args.items,
        matches_path=args.matches,
        manifest_path=args.manifest,
        fixed_oof_path=args.fixed_oof,
