...
def aggregate_weak_specialists(
    *,
    ...
    expected_fold_count = len(np.unique(expected_folds))
    if len(fold_files) != expected_fold_count:
        raise ValueError(
            f"weak specialist aggregate requires {expected_fold_count} fold files, got {len(fold_files)}"
        )
    ...
    if candidate["row_index"].duplicated().any():
        raise ValueError("duplicate weak-specialist OOF row")
    if candidate["row_index"].astype(np.int64).tolist() != dev_rows.tolist():
        raise ValueError("weak-specialist OOF rows do not exactly cover sealed development rows")
    if candidate["fold"].astype(np.int16).to_numpy().tolist() != expected_folds.tolist():
        raise ValueError("weak-specialist fold IDs do not match sealed manifest")
    ...
    payload = {
        ...
        "weak_specialist_oof_macro_ap": float(result["macro_average_precision"]),
        "delta_vs_category_base": float(result["delta_vs_base"]),
        "fold_reports": result["fold_reports"],
        "per_category_ap": result["per_category_ap"],
    }
    (output_dir / "v5e-weak-specialist-metrics.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8"
    )
    candidate.to_parquet(output_dir / "v5e-weak-specialist-oof.parquet", index=False)
    return payload
...
def main() -> int:
    ...
    payload = aggregate_weak_specialists(
        fold_files=sorted(args.fold_dir.rglob("v5e-weak-fold-*-oof.parquet")),
