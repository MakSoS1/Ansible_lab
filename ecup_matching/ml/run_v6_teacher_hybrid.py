...
from .v6_teacher_hybrid import HYBRID_COVERAGES, build_crossfit_hybrid_teacher
from .v6_teacher_student import NON_TEACHER_SIGNAL_NAMES
...
def run_teacher_hybrid(
    *,
    ...
    hybrid = build_crossfit_hybrid_teacher(
        non_teacher,
        six["teacher"],
        categories,
        folds,
    ...
    teacher_score = np.asarray(hybrid["hybrid_teacher_score"], dtype=np.float64)
    teacher_mask = np.asarray(hybrid["teacher_selected"], dtype=bool)
    candidate_scores = {**non_teacher, "teacher": teacher_score}
    target = dev["target"].to_numpy(dtype=np.int8)
    actual_fraction = float(np.mean(teacher_mask))
    print(
        f"[v6-hybrid] coverage={coverage:.2f} rows={len(dev)} teacher_rows={int(teacher_mask.sum())} "
    ...
    category_result = crossfit_category_shrunk_simplex(
        ...
        progress=lambda done, total, fold: print(
            f"[v6-hybrid] coverage={coverage:.2f} phase=category fold={fold} done={done}/{total} elapsed={time.monotonic()-started:.1f}s",
            flush=True,
        ),
    )
    ...
    hgb_result = crossfit_fixed_hgb_stack(
        ...
        folds,
        progress=lambda done, total, fold: print(
            f"[v6-hybrid] coverage={coverage:.2f} phase=hgb fold={fold} done={done}/{total} elapsed={time.monotonic()-started:.1f}s",
            flush=True,
        ),
        **hgb_params,
    )
    ...
    for fold in sorted(np.unique(folds).tolist()):
        ...
        fold_reports.append({
            ...
            "fusion_macro_ap": float(macro_ap_report(frame, fusion[mask])["macro_average_precision"]),
        })

    tag = f"{int(round(coverage * 100)):02d}"
    payload = {
        "version": "v6-teacher-hybrid-v1",
        "coverage": float(coverage),
        "actual_teacher_fraction": actual_fraction,
        "teacher_rows": int(teacher_mask.sum()),
        "architecture": "five real signals + leakage-safe HGB teacher student, overridden by real teacher only on target-free disagreement gate",
        "real_teacher_calibration": "empirical CDF from outer-train teacher OOF only",
        "hybrid_coverages_frozen_before_evaluation": list(HYBRID_COVERAGES),
    ...
    (output_dir / f"v6-teacher-hybrid-{tag}-metrics.json").write_text(
    ...
    pd.DataFrame({
        ...
        "hybrid_teacher_oof_score": teacher_score,
        "student_teacher_oof_score": hybrid["student_teacher_score"],
        "category_shrunk_oof_score": category_oof,
        "hgb_stack_oof_score": hgb_oof,
        "fusion_oof_score": fusion,
    }).to_parquet(output_dir / f"v6-teacher-hybrid-{tag}-oof.parquet", index=False)
