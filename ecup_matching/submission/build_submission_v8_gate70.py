...
EXPECTED_BASE_OOF = 0.598287140395421
EXPECTED_GRAPH_OOF = 0.6000750225512788
...
def main() -> int:
    ...
    if not _close(metadata['base_strict_oof_macro_ap'], EXPECTED_BASE_OOF):
        raise RuntimeError('wrong packaged gate70 base OOF evidence')
    ...
    if len(final) != len(matches) or not np.isfinite(score).all() or len(np.unique(score)) <= 1:
        raise RuntimeError('final gate70+graph prediction is invalid')
    args.output_path.parent.mkdir(parents=True, exist_ok=True)
    final.to_csv(args.output_path, index=False)
    print(f'[v8-gate70] complete rows={len(final):,} coverage={EXPECTED_COVERAGE:.2f}', flush=True)
    return 0
...
def _load_gate70_metadata(path: Path) -> dict[str, object]:
    path = Path(path)
    if not path.is_file() or path.is_symlink():
        raise ValueError('gate70 metadata must be a regular file')
    ...
    if failed:
        raise ValueError(f'invalid gate70 production metadata: {failed}')
    if float(meta['fold_local_graph_strict_oof_macro_ap']) < 0.60:
        raise ValueError('gate70 graph OOF does not clear 0.60 quality gate')
    return meta
...
def build_v8_gate70(
    *,
    source_v6_zip: Path,
    gate70_category_path: Path,
    gate70_hgb_path: Path,
    gate70_metadata_path: Path,
    output_zip: Path,
    ...
    for path in (gate70_category_path, gate70_hgb_path, v8_graph_source, v8_submission_graph_source):
        p = Path(path)
        if not p.is_file() or p.is_symlink():
            raise ValueError(f'package input must be a regular file: {p}')
    gate_meta = _load_gate70_metadata(Path(gate70_metadata_path))

    with tempfile.TemporaryDirectory(prefix='ecup-v8-gate70-build-') as tmp:
        ...
        # Coverage-specific production meta MUST be gate70, never stale gate95.
        shutil.copyfile(gate70_category_path, root / 'model_v6_category_shrunk.json')
        shutil.copyfile(gate70_hgb_path, root / 'model_v6_hgb_meta.joblib')
        shutil.copyfile(gate70_metadata_path, root / 'model_v6_gate_metadata.json')
        ...
        final_meta = {
            ...
            'gate70_metadata_sha256': _sha256(Path(gate70_metadata_path)),
            'gate70_category_sha256': _sha256(Path(gate70_category_path)),
            'gate70_hgb_sha256': _sha256(Path(gate70_hgb_path)),
        ...
        if abs(float(gate_meta['fold_local_graph_strict_oof_macro_ap']) - final_meta['base']['fold_local_graph_strict_oof_macro_ap']) > 1e-12:
            raise RuntimeError('gate70 evidence changed during packaging')
    ...
    if archive_bytes >= 5 * 1024**3:
        ...
        raise ValueError(f'v8 gate70 archive exceeds 5 GiB: {archive_bytes}')
    return {
