...
def _minimal_repo(root: Path, *, version: str = "v2", status: str = "completed") -> None:
    ...
    (root / "docs" / "agent-memory" / "PROJECT_STATE.md").write_text(
        f"Current experiment {version}\n", encoding="utf-8"
    )
    (root / "docs" / "agent-memory" / "EXPERIMENT_INDEX.md").write_text(
        f"| {version} | {status} |\n", encoding="utf-8"
    )
    ...
    current = {
        "version": version,
        "status": status,
        "plan": f"ecup_matching/experiments/{version}/PLAN.md",
        "results": f"ecup_matching/experiments/{version}/RESULTS.md",
        "next_version": "v3",
    }
    (root / "ecup_matching" / "experiments" / "CURRENT.json").write_text(
