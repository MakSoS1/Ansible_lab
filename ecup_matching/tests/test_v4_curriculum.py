...
def test_weak_curriculum_is_disjoint_capped_and_deterministic() -> None:
    human = _frame([(1, 2, 1.0, "Электроника", "human"), (3, 4, 0.0, "Одежда", "human")])
    weak = _frame(
        [
...
def test_weak_curriculum_preserves_serialized_id_text_alignment_when_key_is_reversed() -> None:
    human = _frame([(1, 2, 1.0, "Электроника", "human")])
    weak = _frame([(20, 10, 0.99, "Электроника", "weak"), (30, 31, 0.01, "Электроника", "weak")])
    valid = _frame([(100, 101, 0.0, "Электроника", "human")])
    result = build_weak_curriculum(human, weak, valid, max_weak_rows=2, seed=2026)
    row = result.loc[result["id1"] == 20].iloc[0]
    assert row["id2"] == 10
    assert row["text_a"] == "item 20"
    assert row["text_b"] == "item 10"


def test_weak_curriculum_deduplicates_reversed_pairs_without_corrupting_text() -> None:
    human = _frame([(1, 2, 1.0, "Электроника", "human")])
    weak = _frame(
        [
            (20, 10, 0.90, "Электроника", "weak"),
            (10, 20, 0.99, "Электроника", "weak"),
            (30, 31, 0.01, "Электроника", "weak"),
        ]
    )
    valid = _frame([(100, 101, 0.0, "Электроника", "human")])
    result = build_weak_curriculum(human, weak, valid, max_weak_rows=2, seed=2026)
    pair_rows = result[
        ((result["id1"] == 20) & (result["id2"] == 10))
        | ((result["id1"] == 10) & (result["id2"] == 20))
    ]
    assert len(pair_rows) == 1
    row = pair_rows.iloc[0]
    assert row["target"] == pytest.approx(0.99)
    assert row["text_a"] == f"item {row['id1']}"
    assert row["text_b"] == f"item {row['id2']}"


def test_hard_replay_curriculum_uses_25_25_50_mix() -> None:
    parent = _frame(
        [(100 + i * 2, 101 + i * 2, float(i % 2), "Электроника", "weak" if i >= 8 else "human") for i in range(16)]
    )
    hard = _frame([(1000 + i * 2, 1001 + i * 2, 0.0, "Электроника", "human") for i in range(8)])
    positives = _frame([(2000 + i * 2, 2001 + i * 2, 1.0, "Электроника", "human") for i in range(8)])
    result = build_hard_replay_curriculum(parent, hard, positives, total_rows=16, seed=2026)
