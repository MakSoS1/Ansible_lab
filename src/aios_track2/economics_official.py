...
def compute_calculation(
    ...
    *,
    ...
    conversion_history = _build_conversion_transitions(economic_rows, calculation_start_date, pump_history, econ)
    ...
    for month in sorted(month_rows):
        ...
        item["propertyTaxM"] = (
            (item["residualStartM"] + item["residualEndM"]) / 2 * to_number(econ["propertyTaxRate"]) / 100 / 12
        )
        ...
        item["injectionOpexM"] = item["injectionM3"] * to_number(econ["injectionOpexRubM3"]) / 1_000_000
        item["fundOpexM"] = item["activeWells"] * to_number(econ["fundAnnualRubWell"]) / 12 / 1_000_000
        ...
        item["ebitdaM"] = item["revenueM"] - item["opexM"] - item["deductionsM"] - item["otherIncludedEbitdaM"]
        ...
        item["discountedInflowM"] = (item["revenueM"] + max(0.0, -other_income_expense)) * item["discountFactor"]
    ...
    for year_months in months_by_year.values():
        ...
        for item in year_months:
            item["profitTaxM"] = annual_tax * max(0.0, item["taxableProfitM"]) / positive_base if positive_base > 0 else 0.0
            item["netProfitM"] = item["taxableProfitM"] - item["profitTaxM"]
            item["fcfM"] = item["ebitdaM"] - item["profitTaxM"] - item["capexM"]
            item["chddM"] = item["fcfM"] * item["discountFactor"]
            other_income_expense = item["otherIncludedEbitdaM"] + item["otherExcludedEbitdaM"]
    ...
    for year in sorted(months_by_year):
        ...
        annual_item["profitabilityIndex"] = cumulative_inflow / cumulative_outflow if cumulative_outflow != 0 else 0.0
    ...
    clean_monthly = []
    for item in monthly:
        clean_monthly.append({key: value for key, value in item.items() if not key.endswith("Count") or key in {"startStopCount", "conversionCount"}})
